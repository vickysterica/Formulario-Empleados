<!--FORMULARIO UPDATE VINCULADO AL MULTIFORMULARIO, QUE ACTUALIZARÁ LOS DATOS DE UN EMPLEADO, SEGÚN EL NÚMERO DE EMPLEADO-->
<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
...
<body>
    <?php
        if(isset($_POST["boton_update"])){
            ...
            if(empty($emp_no) || empty($ape) || empty($oficio) || empty($salario) || empty($director) || empty($comision) || empty($dept_no)){
                ...
                $consulta = "UPDATE emple set apellido='$ape', oficio='$oficio', salario='$salario',dir='$director',comision='$comision',dept_no='$dept_no' where emp_no ='$emp_no'";
                $ejecuta_consulta = mysqli_query($conexion,$consulta);

                print "<h2> Se acaba de actualizar lo siguiente </h2>";
                ...
                print 'Número de departamento' .$dept_no.'</p>';
    ...
    ?>
<h2> Formulario de Actualización de empleados </h2>
    <br>
    <form action="multiformulario_update.php" method="POST">
        <div id="izquierda">
        ...
        <p>Introduce el Número de departamento</p>
            ...
            <br>
            <br>
            <input type="submit" name="boton_update" id="boton_update" value="¿Actualizar Empleado?">
        </div>
	</form>
    <?php
}
